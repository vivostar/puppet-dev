...
docker run -d --name hue --hostname hue --network bigtop --privileged -e "container=docker" hue:4.10.1
...
docker exec hue bash -c "echo '$MASTER_IP      master' >> /etc/hosts"
docker exec hue bash -c "echo '$WORKER01_IP      worker01' >> /etc/hosts"
docker exec hue bash -c "echo '$WORKER02_IP      worker02' >> /etc/hosts"
